...
#define _POSIX_VDISABLE (cc_t) 0xFF /* You can’t even generate this * character with 'normal' keyboards.
                                     * But some language specific keyboards can generate 0xFF.
                                     * It seems that all 256 are used, so cc_t should be a short...
                                     */
...
/* Here are the local extensions to the POSIX standard for ProOS. Posix
 * conforming programs are not able to access these, and therefore they are
 * only defined when a ProOS program is compiled.
 */
/* Extensions to the termios c_iflag bit map. */
#define IXANY           0x0800      /* allow any key to continue ouptut */

/* Extensions to the termios c_oflag bit map. They are only active iff OPOST is enabled. */
