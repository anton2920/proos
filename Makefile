...
SUBDIRS = $(BOOTDIR) $(KERNDIR) $(DRIVDIR) $(LIBSDIR) $(THL1DIR)

# TEST
ifdef TEST
SUBDIRS = $(BOOTDIR) $(KERNDIR) $(DRIVDIR) $(LIBSDIR) $(TESTDIR)
endif
...
run: os.img
	...
	/usr/libexec/qemu-kvm -boot order=a -audiodev pa,id=1 -soundhw pcspk -drive if=floppy,format=raw,file=$< &
	sleep 0.1
