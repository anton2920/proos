...
_PROTOTYPE( int sys_sdevio, (int req, long port, int type, int proc_nr, void *buffer, int count));
...
_PROTOTYPE ( int sys_irqctl, (int request, int irq_vec, int policy, int *irq_hook_id) );
...
_PROTOTYPE( int sys_physcopy, (int src_proc, int src_seg, vir_bytes src_vir,
                               int dst_proc, int dst_seg, vir_bytes dst_vir,
                               phys_bytes bytes) );
_PROTOTYPE( int sys_memset, (unsigned long pattern, phys_bytes base, phys_bytes bytes) );

/* Vectored virtual / physical copy calls. */
#if DEAD_CODE /* TODO: library part not yet implemented */
...
_PROTOTYPE( int sys_getinfo, (int request, void *val_ptr, int val_len, void *val_ptr2, int val_len2) );
